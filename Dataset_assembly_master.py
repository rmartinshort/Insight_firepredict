...
def assemble_dataframe_yearjoin():
    ...
    Fires_per_block_year = ADFire.assemble_fires_dataframe_oneperyear(fires_datapath,SF_blocks,SF_yearblocks)
    ...
    e.drop(['GISJOIN_y','Year_y','IDyear_y','LAT_y','LON_y','AREA_y','geometry_y'],axis=1,inplace=True)
    ...
    ALLDATA['LAT'] = ALLDATA['LAT_x'].astype(float)
    ALLDATA['LON'] = ALLDATA['LON_x'].astype(float)
    ALLDATA['AREA'] = ALLDATA['AREA_x'].astype(float)
    ALLDATA['IDyear'] = ALLDATA['IDyear_x'].astype(int)
    ALLDATA.drop(['geometry_x','LAT_x','LON_x','IDyear_x','AREA_x'],axis=1,inplace=True)
    ...
    ALLDATA2.drop('geometry',axis=1).to_csv("Fully_merged_dataset_Autogenerated.csv",index=False)
